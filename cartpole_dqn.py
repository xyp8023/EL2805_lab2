...
###############################################################################
        ...
        if (np.random.uniform() < self.epsilon) or (
                state_actions.all() == 0):
            # non-greedy or when this state has not been discovered
...
###############################################################################
    ...
    def train_model(self):
        ...
        for i in range(self.batch_size):
            ...
            #Allocate s'(i) for the target network array
            # from iteration i in the batch
            done.append(mini_batch[i][4])  #Store done(i)

        target = self.model.predict(update_input)
        #Generate target values for training the inner loop network
...
###############################################################################
        ...
        #Tip 2: What is the Q-value of the action taken
        # at the last state of the episode?
...
if __name__ == "__main__":
    ...
    for e in range(EPISODES):
        ...
        while not done:
            ...
            if done:
                ...
                print("episode:", e, "  score:", score," q_value:", max_q_mean[e],
    ...
    pylab.figure()
    pylab.plot(episodes[-min(100, len(scores)):],
               scores[-min(100, len(scores)):], 'b')
    pylab.xlabel("Episodes")
